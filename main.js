...
const buscarInfo = async  (result) => {
    ...
    const response = await fetch("https://rocakids.com.co/api/kid/scanner/"+id); // PRODUCCION
    ...
    if(registro && registro!=''){
        ...
        document.getElementById('info').innerHTML = `
            <input id="idReg" style="display:none;">
            <h2>Opss!</h2>
            <p>No se encontraron registros con el ficho: ${id}.<br></p>
            <p>O ya se encuentra en estado LEIDO <br><br></p>
            <button id="btnRecargar" class="btn btn-primary" onclick="reload()">Escanear de nuevo</button>
            `;
    }
}
...
const actulizarStatus = async  (Dk) => {
    ...
    const response =  fetch("https://rocakids.com.co/api/kid/scanner/actu/"+id,{
        method:'PUT',
